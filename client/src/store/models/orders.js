...
export const orders = {
  ...
  addOrder: action((state, payload) => [state.data.push(payload)]),
  createOrder: thunk(async (actions, payload) => {
    const { data } = await axios.post("/routes/orders", payload);
  ...
  fetchOrder: thunk(async (actions, payload) => {
    const { data } = await axios.get(`/routes/orders/${payload}`);
